...
export function calculateInventoryForecast(data) {
  ...
  function buildCase(mode) {
    ...
    for (let i=0;i<104;i++) {
      ...
      for (const p of plans) {
        ...
        const target = Number(p.target_volume_nt || 0);
        const caseTarget = mode === 'committed'
          ? ((status === 'committed' || status === 'active') ? target : 0)
          : target * (probabilityPct/100);
        const used = Number(usedByPlan.get(p.id) || 0);
        const remainingPlanTarget = Math.max(0, caseTarget-used);
        let requested = 0;
        if (mode === 'committed') {
          requested = (status === 'committed' || status === 'active') ? gross : 0;
        } else {
          requested = gross * (probabilityPct/100);
        }
        const caseDemand = Math.min(requested, remainingPlanTarget);
        usedByPlan.set(p.id, used+caseDemand);
        planned += caseDemand;
        details.push({customer:p.customers?.name || 'Customer', status:p.customers?.status || '', gross_nt:gross, probability_pct:probabilityPct, target_nt:caseTarget, used_nt:used+caseDemand, case_nt:caseDemand});
      }
    ...
    // Use the first four positive-demand weeks, rather than averaging pre-start zero weeks.
    const positive = weeks.filter(w=>w.planned_nt>0).slice(0,4);
    const activeWeeklyDemand = positive.length ? positive.reduce((s,w)=>s+w.planned_nt,0)/positive.length : 0;
  ...
  const committed = buildCase('committed');
  const weighted = buildCase('weighted');
  return {
    qty,
    totalTarget,
    totalWeightedTarget,
